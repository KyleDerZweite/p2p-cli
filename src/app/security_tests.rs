...
fn authenticated_identity_still_requires_valid_message_state_signature_and_route() {
    ...
    assert!(app
        .validate_incoming(&valid, source)
        .unwrap_err()
        .contains("outside an established"));
    ...
    assert!(app
        .validate_incoming(&tampered, source)
        .unwrap_err()
        .contains("signature verification failed"));
    let other_route = "192.0.2.1:49153".parse().unwrap();
    assert!(app
        .validate_incoming(&valid, other_route)
        .unwrap_err()
        .contains("different session"));
    assert!(app
        .validate_incoming(&signed(&alice, MessageType::TextMessage), source)
        .unwrap_err()
...
fn raw_outgoing_address_never_automatically_trusts_a_new_identity() {
    ...
    for peer in [&bob, &mallory] {
        ...
        let _ = app.handle_network_event(NetworkEvent::MessageReceived(
            signed(peer, MessageType::ConnectionAccept),
            "192.0.2.1:8080".parse().unwrap(),
        ));
        assert!(matches!(app.state.identity_status, IdentityStatus::Unknown));
        assert!(app
            .message_db
            .get_trusted_identity(&peer.get_fingerprint())
            .unwrap()
            .is_none());
    }
}
...
fn invitation_rejects_validly_signed_accept_from_wrong_identity() {
    ...
    let invite =
        crate::network::invitation::Invitation::new(bob.get_public_key_base64(), vec![source])
            .unwrap()
            .encode()
            .unwrap();
    app.start_connection(&invite).unwrap();
    assert!(app
        .validate_incoming(&signed(&mallory, MessageType::ConnectionAccept), source)
        .unwrap_err()
        .contains("does not match invitation"));
    app.validate_incoming(&signed(&bob, MessageType::ConnectionAccept), source)
        .unwrap();
}
...
fn invitation_fallback_preserves_identity_and_discards_stale_failures() {
    ...
    let invite = crate::network::invitation::Invitation::new(
        bob.get_public_key_base64(),
        vec![first, second],
    )
    .unwrap()
    .encode()
    ...
    let _ = app.handle_network_event(NetworkEvent::ConnectionFailed(first, "refused".into()));
    assert_eq!(app.state.peer_ip, Some(second.to_string()));
    assert_eq!(
        app.expected_peer_identity(),
        Some(bob.get_public_key_base64())
    );
    assert_eq!(app.update().unwrap().len(), 1);
    let _ = app.handle_network_event(NetworkEvent::ConnectionLost(first));
    let _ = app.handle_network_event(NetworkEvent::ConnectionFailed(first, "late failure".into()));
    assert_eq!(app.state.peer_ip, Some(second.to_string()));
    assert_eq!(
        app.expected_peer_identity(),
        Some(bob.get_public_key_base64())
...
fn unrelated_connection_loss_preserves_active_chat() {
    ...
    let _ = app.handle_network_event(NetworkEvent::ConnectionLost(
        "192.0.2.1:49153".parse().unwrap(),
    ));
    assert_eq!(app.state.connection_status, ConnectionStatus::Connected);
    assert_eq!(app.state.peer_ip, Some(peer.to_string()));
    assert_eq!(
        app.state.peer_identity_key,
...
fn declined_incoming_request_retains_observed_socket_for_reply() {
    ...
    let peer: std::net::SocketAddr = "192.0.2.1:49152".parse().unwrap();
    let _ = app.handle_network_event(NetworkEvent::MessageReceived(
        signed(&bob, MessageType::ConnectionRequest),
        peer,
    ));
...
fn peer_content_cannot_inject_terminal_controls_or_bidi_overrides() {
    ...
    let mut app = app(alice);
    app.add_message(
        "hello\x1b]52;c;secret\x07\r\u{009b}world\u{202e}abc\u{2066}\nnext".into(),
        MessageSource::Peer,
    );
...
fn paste_bounds_utf8_input_and_filters_controls() {
    ...
    for (mode, limit) in [
        (InputMode::ConnectField, 8192),
        (InputMode::MessageField, 16384),
    ] {
        app.state.input_mode = mode;
        app.handle_ui_event(UiEvent::Paste(format!("\x1b\n\r{}", "😀".repeat(20000))))
            .unwrap();
        let input = if matches!(app.state.input_mode, InputMode::ConnectField) {
            &app.state.connect_input
        } else {
            &app.state.message_input
        };
        assert!(
            input.len() <= limit,
            "input has {} bytes, expected at most {limit}",
            input.len()
        );
...
fn quick_invitation_authenticates_without_implicit_persistent_trust() {
    ...
    let invite =
        crate::network::invitation::Invitation::new(bob.get_public_key_base64(), vec![peer])
            .unwrap()
            .encode()
            .unwrap();
    app.start_connection(&invite).unwrap();
    let _ = app.handle_network_event(NetworkEvent::MessageReceived(
        signed(&bob, MessageType::ConnectionAccept),
        peer,
    ));
    assert_eq!(app.state.connection_status, ConnectionStatus::Connected);
    assert_eq!(
    ...
    assert!(app
        .message_db
        .get_trusted_identity(&bob.get_fingerprint())
        .unwrap()
        .is_none());
}
